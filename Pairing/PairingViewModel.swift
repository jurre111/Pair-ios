...
class PairingViewModel: ObservableObject {
    ...
    private func updateDiscoveredPCs(autoPCs: [String: URL]) {
        ...
        Task { [pcs] in
            ...
            await MainActor.run { [reachable] in
                discoveredPCs = reachable.sorted { $0.name < $1.name }
                reachableHosts = Set(reachable.map { normalizedHost($0.url.host ?? $0.id) })
    ...
    nonisolated private static func filterReachable(pcs: [DiscoveredPC]) async -> [DiscoveredPC] {
        await withTaskGroup(of: DiscoveredPC?.self) { group in
            ...
            var results: [DiscoveredPC] = []
            for await maybePC in group {
                if let pc = maybePC {
                    results.append(pc)
                }
            }
